...
export function PerformanceTimeline({ ranking }: Props) {
  ...
  // Escala das mini barras: relativa ao melhor PPM (ou ao maior do recorte).
  ...
  return (
    <Card as="section" className={styles.panel} aria-labelledby="timeline-title">
      <header className={styles.panelHeader}>
        ...
        <h2 id="timeline-title" className={styles.panelTitle}>Últimos treinos</h2>
      </header>

      {recentResults.length === 0 ? (
        <p className={styles.emptyText}>Seus últimos treinos aparecerão aqui.</p>
      ) : (
