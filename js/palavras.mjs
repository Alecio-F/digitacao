...
let posicaoAleatorias = 0;

export function palavrasAleatorias() {
  const palavra = palavras[posicaoAleatorias];
  posicaoAleatorias = (posicaoAleatorias + 1) % palavras.length;
  return palavra;
}
...
export function palavrasNaOrdem() {
  const palavra = palavrasOriginais[posicaoOrdem];
  posicaoOrdem = (posicaoOrdem + 1) % palavrasOriginais.length;
  return palavra;
}
